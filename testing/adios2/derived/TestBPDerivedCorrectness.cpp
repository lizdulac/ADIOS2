...
TEST_P(DerivedCorrectnessP, ParserTest)
{
    ...
    auto X = bpOut.DefineVariable<float>("@1", {N}, {0}, {N});
    bpOut.DefineDerivedVariable("constAdd", "x = var1 \n x + 1", mode);
    bpOut.DefineDerivedVariable("constMult", "x= var1 ;\n y= \"vars/var_#3\"; 2 * x * y * 5 * 1", mode);
    bpOut.DefineDerivedVariable("Alias1", "x= \"@1\" ;\n y= \"vars/var_#3\"; 5 - (@1 + y * @3 - 3.14159) + add(@4, x) * @5", mode, {"unused","vars/var_#3","fakename","var1","var2","@1"});
