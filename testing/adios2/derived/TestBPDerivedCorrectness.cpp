...
TEST(DerivedCorrectness, MagCurlCorrectnessTest)
{
    const size_t Nx = 25, Ny = 70, Nz = 13;
    float error_limit = 0.0000001f;
    ...
    std::string derivedname = "derived/magcurlV";
    ...
    adios2::IO bpIn = adios.DeclareIO("BPReadMagCurlExpression");
    adios2::Engine bpFileReader = bpIn.Open(filename, adios2::Mode::Read);
    ...
    std::vector<float> readVZ;
    // TODO/DEBUG - VERIFY DATATYPE
    std::vector<float> readMagCurl;

    std::vector<std::vector<float>> calcMagCurl;
    double sum = 0;
    bpFileReader.BeginStep();
    auto varVX = bpIn.InquireVariable<float>(varname[0]);
    auto varVY = bpIn.InquireVariable<float>(varname[1]);
    auto varVZ = bpIn.InquireVariable<float>(varname[2]);
    auto varMagCurl = bpIn.InquireVariable<float>(derivedname);

    bpFileReader.Get(varVX, readVX);
    bpFileReader.Get(varVY, readVY);
    bpFileReader.Get(varVZ, readVZ);
    bpFileReader.Get(varMagCurl, readMagCurl);
    bpFileReader.EndStep();

    float curl_x, curl_y, curl_z, mag;
    float err;
    for (size_t i = 0; i < Nx; ++i)
    {
        for (size_t j = 0; j < Ny; ++j)
        {
            for (size_t k = 0; k < Nz; ++k)
            {
                ...
                */
                ...
                curl_z = -sqrtf(z + 1) * sinf(x) - (2 * expf(2 * y) * sinf(x));
                */
                mag = sqrtf(powf(curl_x, 2) + powf(curl_y, 2) + powf(curl_z, 2));
                if (fabs(mag) < 1)
                {
                    err = fabs(mag - readMagCurl[idx]) / (1 + fabs(mag));
                }
                else
                {
                    err = fabs(mag - readMagCurl[idx]) / fabs(mag);
                }
                sum += err;
            }
        }
    ...
    bpFileReader.Close();
    EXPECT_LT(sum / (Nx * Ny * Nz), error_limit);
}
