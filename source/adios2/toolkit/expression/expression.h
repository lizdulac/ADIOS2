...
public:
  ...
  void add_child(double constant) {};
  Expression* add_child(ExprHelper::expr_op op)
  {
    sub_exprs.push_back({Expression(op),""});
    return &std::get<0>(sub_exprs.back());
  }

  void print()
  {
    std::cout << "Print Expression:" << std::endl;
    std::cout << "\toperation: " << detail.operation << std::endl;
