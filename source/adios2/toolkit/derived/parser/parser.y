              ...
              % union
{
    ...
    void *expr_ptr;
        ...
        verbose % locations
            ...
            % start input % token COMMA L_PAREN R_PAREN ENDL % token FUNCTION % token OPERATOR %
            token INDICES % token NUMBER % token ALIAS PATH % type<dval> NUMBER %
            type<sval> ALIAS PATH INDICES % type<sval> FUNCTION OPERATOR %
            type<expr_ptr> input exp % type<ival> list

            % %

            input : {} |
    ENDL input{} | decl input{} | exp input{/*yyparse_value = $1->expression;*/};
...
| ALIAS INDICES
{
    createExpr(expr_stack, "ALIAS", $1, 0, 0);
    $$ = createExpr(expr_stack, "INDEX", $2, 0, 1);
}
